...
<a href="../05-%20DNP3.md" target="_blank">DNP3</a> documents `map` as a JSON array supplied directly in the `get plc values` / `run plc client`
...
- `namespace` + `name` together identify the policy — this pair is the lookup key, and not coincidentally, it's
  ...
  <a href="../05-%20DNP3.md#dynamic-ingest-with-uns-namespace--master_node" target="_blank">DNP3 — Dynamic ingest with UNS</a>). Using the same
...
- `schema` is exactly the same array documented as `map` in
  <a href="../05-%20DNP3.md#connection-and-map" target="_blank">DNP3 — Connection and map</a> — the same `name`/`type`/`index` (or
...
This is identical to every other `run plc client` example in <a href="../05-%20DNP3.md" target="_blank">DNP3</a> — the only difference is that `map`
...
<a href="../../02-%20Mapping%20Policy.md" target="_blank">Mapping data to tables</a>, used for REST/MQTT ingestion — those policies
...
array shown throughout <a href="../05-%20DNP3.md" target="_blank">DNP3</a> remains simpler.

## See also

- <a href="../05-%20DNP3.md" target="_blank">DNP3</a> — the underlying `get plc values`/`run plc client` command reference
