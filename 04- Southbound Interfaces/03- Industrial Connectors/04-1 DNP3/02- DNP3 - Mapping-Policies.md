...
<a href="../04-%20DNP3.md" target="_blank">DNP3</a> documents `map` as a JSON array supplied directly in the `get plc values` / `run plc client`
...
- `namespace` + `name` together identify the policy — this pair is the lookup key, and not coincidentally, it's
  ...
  <a href="../04-%20DNP3.md#dynamic-ingest-with-uns-namespace--master_node" target="_blank">DNP3 — Dynamic ingest with UNS</a>). Using the same
...
- `schema` is exactly the same array documented as `map` in
  <a href="../04-%20DNP3.md#connection-and-map" target="_blank">DNP3 — Connection and map</a> — the same `name`/`type`/`index` (or
...
<a href="../../../06-%20Networking%20%26%20Security/07-%20Security/01-%20Built-in%20Authentication/01-%20Authentication.md#signing-a-policy" target="_blank">sign → insert</a>
...
<a href="01-%20DNP3%20-%20Deploying%20Connector%20via%20Script.md#prep-policy--build-a-new-mapping-policy" target="_blank">Deploying a DNP3 Connector via Script</a>
...
This is identical to every other `run plc client` example in <a href="../04-%20DNP3.md" target="_blank">DNP3</a> — the only difference is that `map`
...
<a href="../../../08-%20Blockchain%20&%20Metadata/04-%20Mapping%20Policy.md" target="_blank">Mapping data to tables</a>, used for REST/MQTT ingestion — those policies
...
array shown throughout <a href="../04-%20DNP3.md" target="_blank">DNP3</a> remains simpler.

## See also

- <a href="../04-%20DNP3.md" target="_blank">DNP3</a> — the underlying `get plc values`/`run plc client` command reference
- <a href="01-%20DNP3%20-%20Deploying%20Connector%20via%20Script.md" target="_blank">Deploying a DNP3 Connector via Script</a> — a full deployment
