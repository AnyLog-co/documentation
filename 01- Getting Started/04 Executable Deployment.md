...
<<<<<<<< HEAD:00- archive/01- Getting Started/05 Executable Deployment.md
     * `DISABLE_CLI` to _true_ if planned to [run in background](../../02-%20Installation%20&%20Deployment/01-%20Deployment%20Options/07%20Configure%20as%20a%20Service.md)
========
     * `DISABLE_CLI` to _true_ if planned to [run in background](/docs/getting-started/07-configure-as-a-service/)
>>>>>>>> origin/pre-develop:01- Getting Started/04 Executable Deployment.md
```shell
