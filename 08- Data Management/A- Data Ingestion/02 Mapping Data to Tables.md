...
description: ""
layout: page
source_path: "02 Mapping Data to Tables.md"
---
...
Details of the `_bring_` command are available in  [The "From JSON Object Bring" command](../../ORPHANS/x04-southbound-services/json%20data%20transformation.md#the-from-json-object-bring-command) section.  
...
| root          | Bool      |   No      | A true value indicates that the **bring** key is assigned to the root of the source JSON (ignoring the JSON object identified by the **readings** value).  |
| bring         | String    |   Yes     | The key retrieving the blob from the JSON object.  |
...
2) Define a physical database (i.e.: PostgresSQL or SQLite) to the logical database name (london). 
   Details on database configurations are available in the section [Connecting to a local database](../../ORPHANS/x04-southbound-services/sql%20setup.md#connecting-to-a-local-database).
...
Details are available at the [Using EdgeX](../../ORPHANS/x04-southbound-services/using%20edgex.md#using-EdgeX) section of the documentation.  
