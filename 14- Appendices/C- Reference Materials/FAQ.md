...
description: Common questions, known issues, and troubleshooting guidance for AnyLog deployments, plus architecture and edition (AnyLog vs. EdgeLake) reference.
layout: page
---
<!--
## Changelog
...
- 2026-07-14 | Merged in the separate architecture/editions FAQ (previously ZZZ FAQ.md, not actually a duplicate of
              this file — it covered node types, the AnyLog/EdgeLake feature comparison, and deployment
              configuration, none of which overlapped with the troubleshooting content here). Fixed: an empty
              `[here]()` link, a likely-corrupted `archive_json=!00- archive` value appearing twice in the
              example config policy (left flagged rather than guessed), a missing frontmatter block on the
              merged-in content, and several dead relative links. Left the two genuinely unanswered questions
              (EdgeLake vs. Enterprise blockchain abstraction; handling node failures/network partitions) marked
              as not yet documented rather than filled in with invented answers.
-->

This page covers two things: operational troubleshooting for running AnyLog (REST/CORS issues, networking,
data ingestion, queries, Docker/cloud deployment, blockchain/metadata, MCP integration), and architecture/edition
reference (node types, how deployment configuration works, and the AnyLog Enterprise vs. EdgeLake feature
comparison).
...
**Q: What are the different types of nodes in the AnyLog Network?**
All AnyLog containers run the same source code/image — it's the configuration that creates the distinction in
behavior between node types:

- **Master** — an AnyLog instance that simulates the role of a blockchain, serving as a decentralized "oracle"
  for managing metadata, smart contracts, and overall network coordination.
- **Operator** — an AnyLog instance responsible for storing data collected from devices.
  - **Cluster** — a logical policy that defines the relationship between Operator nodes, enabling high
    availability (HA) and informing members of the network about the location and distribution of data. Each
    Operator node belongs to a Cluster, and a Cluster can consist of multiple Operator nodes.
- **Query** — an AnyLog instance dedicated to processing and executing data queries. Any node can serve as a
  Query node if it includes the `system_query` logical database.
- **Publisher** — an AnyLog instance responsible for distributing/publishing data to Operator nodes. Publishers
  typically act as gateways for ingesting real-time data into the network.

> ⚠️ Publisher is not supported in EdgeLake.

**Q: How do I configure an AnyLog node to act as a specific node type?**
Configuration is done through a dotenv file (Docker/Podman) or a YAML file (Kubernetes), in two parts:
- **basic** — standard configuration, such as networking and database settings
- **advanced** — more advanced configuration, such as thread counts and enabling advanced services

Configurations can be found in the [docker-compose repo](https://github.com/AnyLog-co/docker-compose/).

**Q: How does the deployment process work?**
A node is deployed using the following logic:

1. Using the dotenv/YAML configuration file(s), a user defines an AnyLog node type with their preferred configuration.
2. The user deploys the Docker or Kubernetes container with the given configuration.
3. The Docker image downloads a copy of [deployment-scripts](https://github.com/AnyLog-co/deployment-scripts).
4. The user-defined environment variables are converted to AnyLog variables.
5. A configuration policy is declared (if it doesn't already exist) specifying how to configure the node:
...
> **Flagged, not corrected:** the two `archive_json=!archive_json` lines above originally read
> `archive_json=!00- archive` in the source — that `00- ` fragment looks like a stray section-label accidentally
> pasted into the value rather than a real variable name. It's been normalized to `!archive_json` here as the
> most plausible reading (matching the parallel `!compress_json`/`!compress_sql`/`!archive_sql` naming pattern in
> the same line), but this is a guess, not a verified value — confirm against the actual deployment-scripts repo
> before relying on this example as-is.

6. AnyLog deploys/configures the different services based on the configuration policy.

> `!` is the equivalent of `$` for AnyLog/EdgeLake.

**Q: What is the purpose of the Cluster policy and how does it work?**
The cluster policy is a logical policy that defines the relationship between Operator nodes, enabling high
availability (HA) and informing members of the network about the location and distribution of data. Each
Operator node belongs to a Cluster, and a Cluster can consist of multiple Operator nodes.

When an operator receives new data, one of the things it does is check whether the table (and cluster)
definition exists both locally and on the blockchain (as metadata). This information is then used by all other
members of the network to query and share the data.

**Q: How does AnyLog ensure data consistency across Operator nodes?**
Each operator node has a management database that tracks incoming files — where they came from and where they
went (for HA) — guaranteeing consistency, validation, and removal of duplicate data.

**Q: Can a single AnyLog node perform multiple roles at once?**
A node can perform different roles based on the enabled services and databases it's connected to. The only
limitation: the Operator and Publisher services cannot be enabled on the same instance.

**Q: How is data partitioned in AnyLog, and why do some tables have the `par_` prefix?**
AnyLog/EdgeLake partitions data automatically; this can be disabled or customized in the configuration file.
When executing a query, AnyLog/EdgeLake scans only the relevant partition rather than the entire table, making
queries significantly more efficient.

> Details were previously linked to `metadata requests.md#creating-data-tables`, which no longer resolves in the
> current doc tree. The likely successor is the table-creation/schema content — confirm and re-link rather than
> trusting this note.

**Q: What are the hardware/resource requirements for running different node types?**
...
| CPU | Intel, ARM, and AMD supported; single-CPU machines up to large multi-core servers (including gateways, Raspberry Pi, etc.) |
| Storage | Horizontal scaling via dynamically added nodes; storage needs depend on data volume/retention per node; automated archival and transfer to larger nodes supported |
| Network | TCP-based network required (local, internet, or hybrid); overlay networks recommended (Nebula is the default); static IP and 3 open ports accessible per node (via overlay or direct network) |
...
> The original "further details" link (`training/prerequisite.md`) doesn't resolve in the current tree — not
> replaced here since no clear successor was found.

**Q: How do I monitor the health and status of nodes in the AnyLog network?**
AnyLog (and EdgeLake) provide alert and monitoring capabilities at both the machine level (CPU, RAM, network I/O,
disk usage) and the data level.

> The original link (`alerts and monitoring.md`) doesn't resolve in the current tree. This is likely covered
> under `06- Data Management/D- Monitoring & Alerts/` — confirm the specific page before re-linking.

**Q: What is the difference between blockchain abstraction in EdgeLake and AnyLog Enterprise?**
*(Not yet documented.)*

**Q: How do I handle node failures or network partitions?**
*(Not yet documented.)*

**Q: Is it possible to upgrade EdgeLake to AnyLog Enterprise?**
Since AnyLog builds on EdgeLake and a node's services are based on user-defined configuration, the upgrade is as
simple as updating the image name in the `docker-compose` file from `anylogco/edgelake` to
`anylogco/anylog-network`.

> Directions for upgrading: *(not yet documented)*.
...
See [Using REST](../08-%20Northbound%20Connectors/A-%20Direct%20Connectors%20Generic/Using%20REST.md) for full examples and Python code.
...
See the MCP & AI Integration documentation for setup details. *(Original link used old Jekyll-relative-URL syntax pointing at `/docs/docs/mcp-ai-integration/` — doesn't resolve as a real path in this tree; not replaced here since no clear current-tree successor was found.)*