...
The data generator generates data in JSON format and delivers the the data to nodes using REST (PUT or POST) requests or using a PUBLISH request.    
* If the data generator is configured to use PUT request, the data delivered is assigned to the table without modifications.
* If the data generator is configured to use POST request, the data delivered can be modified by associating
   the data to a mapping rule (on the command line) or to a [mapping policy](../mapping%20data%20to%20tables.md#mapping-data).
  
## Prerequisites
* An AnyLog Operator node with the REST service enabled (the IP and Port assigned to the REST service will be configured on the data generator as the destination IP and Port).
* For a POST request do the following:
    * Include a topic in the message header.
    * Assign the topic to a mapping role or a mapping policy:
        * Example of a mapping role
        * Example of a mapping policy
* If the data generator is configure to PUBLISH data:
    * The Broker service needs to be configured on the AnyLog node.
    * Assign the topic to a mapping role or a mapping policy.
    
 ## The tables generated by the data generator
