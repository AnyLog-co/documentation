...
When a node is deployed, users select the services that the node will be offering.  
These services together with policies associated with the node determine how the node operates.  

We provide a Docker based install to 5 types of AnyLog node. These types of nodes are different by the services they offer.
Note that it is simple to enable or disable services on nodes, even if the nodes are deployed and opertional.

This document provides general directions for deploying an AnyLog node, via _Docker_. of the following types of nodes:

* **Generic** - An AnyLog node that offers most of the optional services. These services include _TCP_ (a communication service that makes 
  the node a member of the network), _REST_ (a networking service supporting communication with 3rd parties applications) and
_Broker_ (a service allowing to publish data on the node). 
* **Master** – A node that manages the shared metadata (if a blockchain platform is used, this node is redundant).
* **Operator** – A node that hosts the data. 
* **Query** – A node that coordinates the query process. 
* **Publisher** - A node that supports distribution of data from device(s) to operator nodes. 

### Passing variables to the deployment process
Using **system variables** uses can assign values to keys. These keys are referenced in the node configuration process
to apply a configuration option or a configuration value.
  
### Basic Deployment
Our basic deployment is a node of type Generic. The deployment does not require any user-defined configurations,  
...
The script updates the AnyLog configuration file for either Docker or Kubernetes, along with the `.env` file for 
Docker deployment.
