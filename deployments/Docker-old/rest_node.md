...
A REST deployment is an AnyLog node that has all the variables declared, as well as connecting to _TCP_, _REST_ and
_Broker_ (if declared). The REST deployment is our base deployment, default ports: 2148 (_TCP_) and 2149 (_REST_), when
deploying without any configurations.
...
1. Fill-out the Anylog configurations file for [rest node](https://github.com/AnyLog-co/deployments/blob/master/docker-compose/anylog-rest/anylog_configs.env)
...
File path: `$HOME/deployments/docker-compose/anylog-rest/anylog_configs.env`
```dotenv
# --- General ---
...
File Path: `$HOME/deployments/docker-compose/anylog-rest/.env`
```dotenv
BUILD=predevelop
