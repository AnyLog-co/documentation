...
For a step-by-step process of what's being done in the background, please visit the [deployment process](../Docker/master_node_deployment_process.md)
...
```dotenv
# --- General ---
# Information regarding which AnyLog node configurations to enable. By default, even if everything is disabled, AnyLog starts TCP and REST connection protocols [Default: master]
NODE_TYPE=master
...
ubuntu@master-node:~$ docker attach --detach-keys=ctrl-d master-node 
