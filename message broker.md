...
* Executing `run mqtt client` command dedicates a thread (_client_ to the MQTT broker) to process subscribed messages. 
...
```anylog
<run mqtt client where broker = local and user = mqwdtklv and password = uRimssLO4dIo and topic = (
    name = test and 
    dbms = "bring [metadata][company]" and 
    table = "bring [metadata][machine_name] _ [metadata][serial_number]" and 
    column.timestamp.timestamp = "bring [ts]" and 
...
Usage:
```anylog
run mqtt client where broker = rest and user-agent = anylog and [Config parameters] and topic = (topic 1 params) and topic = (topic 2 params) .... 
```
