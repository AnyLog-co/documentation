...
See [Get Commands](/docs/getting-started/get-commands/) for the full reference.
...
For a consolidated guide covering continuous monitoring, scheduled tasks, streaming alerts, and aggregator setup, see [Continuous Monitoring with Automated Alerts](/docs/data-management/monitoring-alerts/continuous-monitoring-with-automated-alerts/).
...
* To support continues monitoring, monitoring tasks are placed on the _scheduler_. The scheduler functionality is explained at [Alerts and Monitoring](/docs/data-management/monitoring-alerts/alerts-and-monitoring/#alerts-and-monitoring).
...
* The command `get os process all` provides info on all processes.
...
or [stored on a local database](#organizing-node-status-in-a-database-table).  
...
Usage:
```anylog
get status where format = [reply format] and include [a dictionary key]
```
Details:  
...
disk_free = get disk free d:disk_percentage = get disk percentage d:```
...
AL anylog-node > get processes
    ...
    MQTT           |Running     |                                                                     |
    Message Broker |Running     |Listening on: 24.23.232.144:7850 and 10.0.0.78:7850, Threads Pool: 6 |
    SMTP           |Not declared|                                                                     |
...
The scheduler runs tasks periodically without manual intervention. See [Background Services — Scheduler](/docs/core-concepts/background-processes/#scheduler) for how to start it.
...
* [Streamer]((/docs/core-concepts/background-processes/#streamer)) such that the data is flushed to disk.
* [Operator]((/docs/core-concepts/background-processes/#operator-service)) for the flushed data to be ingested to the table.
* [Scheduler](/docs/data-management/monitoring-alerts/alerts-and-monitoring/#invoking-a-scheduler) to process scheduled tasks.
...
Note: Partition command is detailed [here]((/docs/commands-cli/command-categories/anylog-commands/#partition-command)).
...
Note:
* Drop partition command is detailed [here]((/docs/commands-cli/command-categories/anylog-commands/#drop-partition-command)).
...
A node that updates a status associated to a monitored topic will be continuously included in the output of
...
In the examples below, the monitoring commands are assigned to the scheduler for continuous monitoring (on each participating/monitored node).
...
Streaming conditions evaluate incoming data in real time and trigger actions when conditions are met.
...
See [Background Services — SMTP](/docs/core-concepts/background-processes/#smtp-client) for details.
...
**Usage**:
```anylog
continuous [time] [list of commands (comma separated commands)]
```
...
See [Get Commands — Resource monitoring](/docs/getting-started/get-commands/#resource-monitoring) for the full list.
