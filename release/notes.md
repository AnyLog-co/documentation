...
### New Features
1. New commands: ```get global tables``` and ```get local tables``` See details under [The metadata](/docs/sql-setup/#the-metadata) 
2. Add **include** and **exclude** options to the command ```blockchain get root policies``` - See details [here](/docs/blockchain-commands/#filtering-root-policies)

AnyLog Version: 1.4.2510-beta3 [e0d9e1] [2026-03-08 17:52:24]
### New Features
1. New options for issuing AnyLog commands via REST: 
    * [Specifying commands in the message URL](/docs/using-rest/#specifying-commands-in-the-message-url)
    * [Specifying commands in the message body](/docs/using-rest/#specifying-commands-in-the-message-body)

AnyLog Version: 1.4.2510-beta3 [8ca403] [2026-02-02 16:13:21] [Details](/docs/blockchain-commands/#retrieving-root-policies)
### New Features
...
3. Adding UNS policies generation. [Details](/docs/message-broker/#generating-a-unified-namespace-uns-policies)
...
1. New option for where condition to retrieve immediate child ```childfrom``` command. [Details](/docs/anylog-commands/#conditional-execution)
...
### New features
1. (AE) Manage Windows Event Log. Detaile [Run Scheduled Pull](/docs/scheduled-pull/)
...
2. (AE) New command: **get nics list** - Retrieves and displays a list of all network interfaces (NICs). [Details](/docs/network-configuration/#get-the-list-of-nics).
3. (AE) New command: **set internal ip with [nic name]**. [details](/docs/network-configuration/#set-internal-ip-via-network-interface)
...
### New features
1. (AE) Adding EtherNet/IP connector. See details in [EtherNet/IP](/docs/enthernetip/)
### Updates
1. (AE) Making the OPCUA call consistent with EtherNet/IP - [opcua](/docs/opcua/)

## AnyLog Version: 1.3.2504 [673f50] [2025-04-27 20:15:35]
### New features
1. (AE) Adding **extend** and **include** options to the Grafana payload. See details in [example 2 in Using Grafana documentation](/docs/northbound-connectors/using-grafana/#using-the-time-series-data-visualization).
...
### New Features:
1. (AE) Optimized increments function. Details: [Increments Optimized Version](/docs/queries/#increments-optimized-version)  
2. (AE) A new AnyLog command: **get increments params**. Details: [get-increments-params-command](/docs/queries/#the-get-increments-params-command)
3. (AE) A new option to optimize data points returned in Grafana. Details: [Using the Time-Series Data Visualization](/docs/northbound-connectors/using-grafana/#using-the-time-series-data-visualization).
...
2. (AE) New Functionality for String Substring Operations. Details: [Special Bring Values](/docs/json-data-transformation/#special-bring-values)
3. (AE) Severe error messages (boxed) are now printed on the node in RED.

## AnyLog Version: 1.3.2401 [d9321d] [2025-03-07 18:19:06]
### New Features:
1. (AE) Using DNS names. Details: [Get DNS Name](/docs/anylog-commands/#get-dns-name)

## AnyLog Version: 1.3.2401 [d6c050] [2025-02-16 13:59:47]
### New Features:
1. (A) Aggregations over user data. Details: [Aggregations](/docs/aggregations/)
2. (AE) New AnyLog command: **subprocess** to run shell scripts.

## AnyLog Version: 1.3.2401 [6665e9] [2025-01-26 14:14:09]
### New Features:
1. (AE) wait for a blockchain sync. Details: [AnyLog Commands](/docs/anylog-commands/#the-wait-command)
...
```
### Changes
1. (AE) Modify the command **exit mqtt** to **exit msg client [n/all]** - Details are in: [Processing messages and terminating a subscription](/docs/message-broker/#processing-messages-and-terminating-a-subscription).

## **Version** [c03b82] [2025-01-02 12:30:17]
### New Features:
1. (AE) OPCUA support. Details: [OPCUA](/docs/opcua/) 

## **Version**: [024a85] [2024-12-21 13:31:31]
### New Features:
1. (AE) A new command: **file to** - write a file to a specified directory, using CLI or via REST. 
   Details are in [Copy a file to a folder](/docs/file-commands/#copy-a-file-to-a-folder) section.  
   This command can be used to remotely copy configurations to a node. An example is available in the [using rest](/docs/using-rest/#examples) section.
...
1. (AE) The required structure for the file name in the **file store** command is optional. 
   See details in the [Insert a file to a local database](/docs/image-mapping/#insert-a-file-to-a-local-database) section. 

## **Version**: [a4924f] [2024-12-07 16:49:46] /
### New Features:
1. (AE) HTTP Commands - specifying commands and output format using http requests. Details in [http commands](/docs/http-commands/). 

## **Version**: [be71d3] [2024-08-29 15:18:04] |
### New Features:
1. (AE) New Casting features in SQL: function, lstrip, rstrip, timediff. Details in [CAST Data](/docs/queries/#cast-data)
2. (AE) Increment function without specifying the time range - the time intervals would be provided dynamically.  
3. (AE) Configuring the number of threads when a message to peers is send. See details [here](/docs/node-configuration/#configuring-the-number-of-threads-supporting-message-send-to-peer-nodes).
...
### New Features:
1. (AE) New command: **get policies diff** detailed in [Compare Policies](/docs/policies/#compare-policies).

## **Version**: 1.3.240112 | 

### New Features
1. (AE) Adding support to gRPC [Using gRPC](/docs/using-grpc/)
2. (AE) Deprecated: "run mqtt client" --> Replaced by: "run msg client" 
3. (AE) **delete archive** command. Details: [Operator data archival](/docs/background-processes/#operator-data-archival). 
4. (AE) Adding support to syslog [Using SysLog](/docs/using-syslog/)

## **Version**: 1.3.23110 | **Release Date**: Oct.  12 2023 (Official)

### New Features
1. (AE) Adding sort by columns to **blockchain get** command. Details: [the bring command](/docs/json-data-transformation/#the-bring-keyword)
2. (AE) Adding sort by columns to **get data nodes** command. Details: [View the distribution of data to clusters](/docs/high-availability/#view-the-distribution-of-data-to-clusters)

## **Version**: 1.3.2309 | **Release Date**: Oct.  2 2023 (Official)

### New Features:
1.  (AE) pip Install. Details: [pip Install](/docs/training/advanced/pip-install/)
2.  (AE) Deploy AnyLog node as a background process. Details: [Deploy AnyLog as a background process](/docs/training/advanced/background-deployment/).
3.  (AE) Map a local CLI to a peer node. Details: [Assigning a CLI to a peer node](/docs/training/advanced/background-deployment/#assigning-a-cli-to-a-peer-node).
4.  (AE) Start a new node with a seed from a peer node. Details: [Retrieving the metadata from a source node](/docs/blockchain-commands/#retrieving-the-metadata-from-a-source-node)
5.  (AE) [Associating peer replies to a key in the dictionary](/docs/network-processing/#associating-peer-replies-to-a-key-in-the-dictionary).    
6.  (AE) wait command - pauses execution by time and condition. Details: [The Wait Command](/docs/anylog-commands/#the-wait-command)
7.  (AE) Create policy command - a command option to declare policies and include default attributes. Details:
    [Creating policies using the Create Policy command](/docs/policies/#creating-policies-using-the-create-policy-command)
...
1.  (AE) The command: **run blockchain sync where source = master**, if connection info is not provided, connection info is
    retrieved from the policy of the Master Node. Details: [Blockchain Synchronizer](/docs/background-processes/#blockchain-synchronizer).
...
4. (AE) Option for **min** and **max** values in the ```bring command```: Details: [The bring keyword](/docs/json-data-transformation/#the-bring-keyword)
