...
Additional information is available at the [configuring a local database](/docs/sql-setup/#configuring-a-local-database) section.
...
The command [file get](/docs/file-commands/#file-copy-from-a-remote-node-to-a-local-node)
...
2) An Operator node configured with the following functionalities:
    * As a message broker - the new data is published on AnyLog node as a Broker.  
      ...
      ```
      Details on configuration of AnyLog as a broker are available [here](/docs/message-broker/#configuring-an-anylog-node-as-a-message-broker).
    * As a Blobs Archiver - this process loads the images into blobs database (such as MongoDB).  
      ...
      ```
      Details on configuration of the Blobs Archiver process are available [here](/docs/background-processes/#the-blobs-archiver).
