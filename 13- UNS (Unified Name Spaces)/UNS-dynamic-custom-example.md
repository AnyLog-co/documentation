...
The [Unified Namespace](UNS.md) page introduces `dynamic=true` using single-value, single-topic examples (one
...
This sits between the two approaches described in [Unified Namespace](UNS.md#auto-generated-vs-user-defined):
still auto-generated (no `blockchain insert` of hand-authored policies, as in
[Custom UNS (data stream, ISA-95)](UNS-custom.md)), but with enough explicit column mapping that the resulting
...
This pattern sits deliberately between the two poles described in [Unified Namespace](UNS.md), and the tree
...
approach in [Custom UNS (data stream, ISA-95)](UNS-custom.md) — while still using `dynamic=true` for the raw
...
- [Unified Namespace](UNS.md) — the base `dynamic=true` model (scalar values, MQTT/OPC-UA)
- [Custom UNS (data stream, ISA-95)](UNS-custom.md) — the fully hand-authored, policy-based approach