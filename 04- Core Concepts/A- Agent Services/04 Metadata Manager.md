...
# Using a Metadata Manager (Master) Node

## Overview

A Metadata Manager (Master) Node stores the metadata information in a local database and serves as a replacement to a blockchain.    
Any member node can serve as a Metadata Manager (Master) - a Metadata Manager (Master) Node is a node that is considered by members of the network as a node that maintains an updated copy of the metadata.      
That means that member nodes are in agreement to reflect updates of the metadata with the Metadata Manager (Master) Node and nodes will update their local copy of the metadata from the metadata maintained by the Metadata Manager (Master) Node.

## Configuring a Metadata Manager (Master) Node

The Metadata Manager (Master) Node maintains the metadata using a local database - the metadata information is stored in a database called `_blockchain_` and a table called `_ledger_`.
...
## Updating the metadata on a Metadata Manager (Master) Node

Nodes in the network need to update the Metadata Manager (Master) Node with changes to the blockchain.  
Updating the Metadata Manager (Master) Node with changes to the metadata is by issuing the following command:
```anylog
run client (destination) blockchain push [JSON data]
```
`_destination_` - the IP and Port of the Metadata Manager (Master) node.
`_JSON data_` - the policy to update.

Additional information on the blockchain commands is available at [blockchain commands](/docs/core-concepts/agent-services/blockchain-commands/).
...
A node can enable a synchronization process. This process periodically pulls the blockchain data from the local database of the Metadata Manager (Master) Node to the local blockchain file on the target node (the node that is executing the sync process).    
The synchronization process is detailed at [blockchain synchronizer](/docs/core-concepts/background-processes/#blockchain-synchronizer).
...
description: ""
layout: page
source_path: "ZZZ metadata requests.md"
---
# Metadata Requests
...
An overview on the global metadata layer is provided in [Managing Metadata](/docs/core-concepts/metadata-management/#managing-metadata).  
...
Note: The processes of a node in the network are agnostic to where the global metadata is hosted (a blockchain or a Metadata Manager (Master) node). 
References to the blockchain should be interpreted as references to the metadata that can be hosted using a blockchain as well as a Metadata Manager (Master) node.
...
A description of the directory structure of a node is available at the section [Local Directory Structure](/docs/getting-started/#local-directory-structure).  
The section [Adding Data](/docs/data-management/data-ingestion/adding-data/) 
details how watch-directories are used and how data is added to nodes in the network using REST.       
The section [Using MQTT Broker](/docs/connectors-integrations/messages-brokers/message-broker-setup/#using-a-message-broker) details how data is added using a broker.  
...
Note: Details on the TSD tables is available at [Managing Data files](/docs/data-management/monitoring-alerts/managing-data-files-status/).
...
A node may manage data locally. The command `get databases` lists the databases that are serving data on the local
node.  
