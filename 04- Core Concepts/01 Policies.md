...
blockchain platform (like Ethereum) or contained in a Metadata Manager (Master) node. Regardless of where the blockchain is hosted, every 
...
Metadata Manager (Master) node is used, the Metadata Manager (Master) node is configured such that the ledger is stored on a local database.
...
```
**policy type** - The type of the policy created (i.e.: Metadata Manager (Master), operator, query, publisher).  
...
1. Retrieve the IP and Port of the Metadata Manager (Master) node.
