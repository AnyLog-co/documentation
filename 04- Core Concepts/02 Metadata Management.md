...
The document [metadata requests](../ORPHANS/x04-southbound-services/metadata%20requests.md#metadata-requests) 
...
* A Matadata Manager (Master) node - A member node in the AnyLog network that maintains a complete copy of the Metadata in a local database.
...
When a node joins the network, the node downloads the existing metadata from a Matadata Manager (Master) Node or from the blockchain.  
...
* Using a Matadata Manager (Master) Node - By sending updates to the Matadata Manager (Master) Node and receiving updates from the Matadata Manager (Master) Node.

## Policies

Policies are rules and logic that determine how nodes in the network operate. 
The policies are placed on the global metadata layer and become available to the nodes of the network with the blockchain/Matadata Manager (Master)-node synchronization process.  
...
The attributes _id_ and _date_ are added dynamically when the policy is updating the blockchain or the Matadata Manager (Master) node.    
...
The blockchain command are issued to a metadata repository which can be on the local node (either as a JSON file or hosted in a database), a Matadata Manager (Master)-node or a blockchain platform.  
 
The _blockchain commands_ are detailed in the [blockchain commands section](../ORPHANS/x04-southbound-services/blockchain%20commands.md).
...
### Updating a Matadata Manager (Master) node

The following command updates a Matadata Manager (Master) node with a new policy:

```anylog
blockchain push [policy] 
```

If the update is done from a node which is not the Matadata Manager (Master) node, the update needs to be sent as a message to the Matadata Manager (Master) node as in the example below:

```anylog
run client !master_node blockchain push !test_policy
```

### Retrieve metadata from a Matadata Manager (Master) node
...
Examples:  
To retrieve data from the Matadata Manager (Master) node:  
* Use the command `blockchain pull` to pull the log file from the Matadata Manager (Master) node. The output file will be paced in the blockchain directory with the following options:
...
* Use the command `file get` to copy the output file (of the pull command) from a node (like the Matadata Manager (Master) node) to a destination node.
```anylog
file get [source location] [destination location]
```
[source location] - is the path name and file name on the Matadata Manager (Master) node.  
[destination location] - is the path name and file name on the requesting node.

The following script pulls the metadata from a Matadata Manager (Master) Node and copies the log file to the local node to serve as the metadata on the local node.
```anylog
run client [ip]:[port] "blockchain pull to log"    # create a copy of the log file on the Matadata Manager (Master) Node
run client [ip]:[port] "file get [source path and file name] [destination path and file name] # copy the log file from the Matadata Manager (Master) Node to the Local Node.
```
In the above script:
`[ip]:[port]` - the ip and port of the Matadata Manager (Master) Node.
`[source path and file name]` - the path and file name of the log file on the Matadata Manager (Master) Node.
`[destination path and file name]` - the path and file name of the metadata log file on the Local Node.

## Periodically synchronizing the local copy of the metadata with a blockchain or a Matadata Manager (Master) node
