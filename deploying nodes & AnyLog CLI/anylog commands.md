...
| Commands                                                                                              | 
|-------------------------------------------------------------------------------------------------------|
| [alerts](../monitoring/alerts%20and%20monitoring.md#alerts-and-monitoring)                            |
| [backup](../deploying%20nodes%20&%20AnyLog%20CLI/anylog%20commands.md#backup-command)                 | 
| [blockchain](../blockchain/blockchain%20commands.md)                                                  | 
| [drop partition](../deploying%20nodes%20&%20AnyLog%20CLI/anylog%20commands.md#drop-partition-command) | 
| [file](../deploying%20nodes%20&%20AnyLog%20CLI/file%20commands.md)                                    | 
| [get](../deploying%20nodes%20&%20AnyLog%20CLI/anylog%20commands.md#get-command)                       |
| [partition](../deploying%20nodes%20&%20AnyLog%20CLI/anylog%20commands.md#partition-command)           | 
| [rest](../deploying%20nodes%20&%20AnyLog%20CLI/anylog%20commands.md#rest-command)                     | 
| [set](../deploying%20nodes%20&%20AnyLog%20CLI/anylog%20commands.md#set-command)                       | 
| [sql](../deploying%20nodes%20&%20AnyLog%20CLI/anylog%20commands.md#sql-command)                       | 
| [test](../deploying%20nodes%20&%20AnyLog%20CLI/anylog%20commands.md#sql-command)                      | 
| [time file](../monitoring/managing%20data%20files%20status.md#Time-file-commands)                     | 
...
| [set query log on/off](../monitoring/logging%20events.md#the-query-log)                                                                            | Enable/Disable a log to record the executed queries. |
| [set query log profile [n] seconds](../monitoring/logging%20events.md#the-query-log)                                                               | Applying the Query Log to queries with execution time higher than threshold.  |
...
| [set reply ip = [ip]](../deploying%20nodes%20&%20AnyLog%20CLI/network%20configuration.md#setting-a-different-ip-address-for-replies) | Set the IP address that for a reply message. |
| [set self ip = [ip]](../deploying%20nodes%20&%20AnyLog%20CLI/network%20configuration.md#self-messaging)                             | Set the IP address when the sender and receiver are the same node. |
...
| [set streaming condition](../deploying%20nodes%20&%20AnyLog%20CLI/streaming%20conditions.md#condition-declaration)                                 |  Declare a condition on streaming data. |
...
| reset reply IP | Identify a reply IP to be used by the replying node. Details are available at [Network Configuration](../deploying%20nodes%20&%20AnyLog%20CLI/network%20configuration.md#reset-the-reply-ip-to-the-source-ip).|
| reset self IP | Identify an IP to be used when sender and receiver are the same node. Details are available at [Network Configuration](../deploying%20nodes%20&%20AnyLog%20CLI/network%20configuration.md#reset-self-messaging).|
...
|-----------------------------------------------------------------------------------------------------------------------------------------------------| ------------| 
| [get event log](../monitoring/logging%20events.md#the-event-log)                                                                                    | Records the Last commands processed by the node. | 
| [get error log](../monitoring/logging%20events.md#the-error-log)                                                                                    | Records the last commands that returned an error. Adding a list of keywords narrows the output to error events containing the keywords.|
...
| [get query log](../monitoring/logging%20events.md#the-query-log)                                                                                    | The last queries processed by the node. Enable this log using the ***set query log*** command|
...
| [get operator](../monitoring/monitoring%20calls.md#get-operator)                                                                                    | Information on the Operator processes. |
...
| [get streaming](../monitoring/monitoring%20calls.md#get-streaming)                                                                                  | Information on streaming data from REST and MQTT calls. |
...
| [get rest calls](../monitoring/monitoring%20calls.md#get-rest-calls)                                                                                | Statistical information on the REST calls. |
| [get rest server info](../monitoring/monitoring%20calls.md#rest-server-configuration)                                                               | Information on the REST server configuration. |
| [get msg clients](../monitoring/monitoring%20calls.md#get-msg-clients)                                                                              | Information on clients subscribed to topics. |
...
| [get status](../monitoring/monitoring%20nodes.md#the-get-status-command)                                                                            | Replies with the string 'running' if the node is active. Can be extended to include additional status information | 
...
| [get rows count](../monitoring/monitoring%20nodes.md#monitoring-data-commands)                                                                      | Details the number of rows in all or specified tables.|
...
| get datetime [date-time function](../query%20data/queries.md#get-datetime-command)                                                                  | Translate a date-time function to the date-time string. |
...
| get monitored | Retrieve the list of topics monitored by an aggregator node. Details are available [here](../monitoring/monitoring%20nodes.md#organizing-nodes-status-in-an-aggregator-node).         |
| get monitored [topic] | Retrieve monitored info on a specific topic from an aggregator node. Details are available [here](../monitoring/monitoring%20nodes.md#organizing-nodes-status-in-an-aggregator-node). |



Additional information is available at [monitoring nodes](../monitoring/monitoring%20nodes.md#monitoring-nodes).
...
detail are provided in the [Query options](../query%20data/queries.md#query-options) section.
...
Details on queries executed against time series data are available in [Optimized time series data queries](../query%20data/queries.md#optimized-time-series-data-queries).
### Monitoring queries  
Details on profiling and monitoring queries are available in [Profiling and Monitoring Queries](../query%20data/profiling%20and%20monitoring%20queries.md#profiling-and-monitoring-queries)
...
Users can leverage the [backup](../deploying%20nodes%20&%20AnyLog%20CLI/anylog%20commands.md#backup-command) process prior to the drop of the partition.
