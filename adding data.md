...
* POST - data is provided in JSON format, the data includes a topic that determines the mapping to the table structure.
  Details are available in the section [AnyLog as a broker receiving REST commands](/docs/message-broker/#anylog-as-a-broker-receiving-rest-commands).
...
Configuring a node as a _rest server_ is detailed at [REST requests](/docs/background-processes/#rest-requests).  
...
Details on how an operator is configured are available [here](/docs/background-processes/#publisher-process).
...
get streaming
```
Command details are available [here](/docs/monitoring-calls/#get-streaming).
...
in the [Subscribing to REST calls](/docs/using-rest/#subscribing-to-rest-calls) section.
...
Details are available at the [message broker section - Subscribing to a third party broker](/docs/message-broker/#subscribing-to-a-third-party-broker).

## Configuring the AnyLog node as a message broker

Details are available at the [message broker section - Configuring an AnyLog node as a message broker](/docs/message-broker/#configuring-an-anylog-node-as-a-message-broker).
...
get msg client
```
Command details are available [here](/docs/monitoring-calls/#get-msg-clients).
...
get rest calls
```
Command details are available [here](/docs/monitoring-calls/#get-rest-calls).
...
File naming convention is detailed at [Managing Data files](/docs/managing-data-files-status/).

The following command provides status and statistics on the processing of the JSON files:
```anylog
get operator
```
Command details are available [here](/docs/monitoring-calls/#get-operator).
